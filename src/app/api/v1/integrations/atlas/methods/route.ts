...
/**
 * GET /api/v1/integrations/atlas/methods — canonical, vendor-neutral
 * nexus-atlas-read/v1 payload (no prices, no commercial terms, no
 * equivalence verdicts; see @/lib/integrations/atlas).
 */
export const GET = withApi("v1/integrations/atlas/methods", async () => {
  const repo = await getRepository();
  const payload = await buildAtlasResponse(repo, "methods");
