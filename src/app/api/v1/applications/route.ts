...
/** GET /api/v1/applications — list applications. Filter: q. */
const querySchema = listQuerySchema.extend({
  q: z.string().trim().min(1).optional(),
});

export const GET = withApi("v1/applications", async (request) => {
  ...
  const paged = await repo.list(
    "application",
    toListParams(parsed.data, ["name", "createdAt"], { query: parsed.data.q }),
  );
