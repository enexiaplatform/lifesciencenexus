...
- Completed: `src/lib/demo/` — 763 synthetic records, all `isDemo: true`, fictional names; portfolio spans the upstream → downstream → QC chain for Pharma API and Biopharma (cell culture media, process chemicals, chromatography, filtration, endotoxin testing) plus the industrial-microbiology Vietnam wedge (culture media, QC organisms, EM/sterility equipment, tenders in all three pipeline states, installed base); every category shelf offers a two-brand choice on the `/categories` browse pages; wired `DemoRepository` (tenant isolation `tenant_demo` vs `tenant_other`, live-computed signals, duplicate candidates via real engine).
